...
import torch
import sys
sys.path.append("/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/code")
from header import *
...
from MotionDiffuse.trainers.ddpm_trainer_copy import DDPMTrainer
...
sys.path.append("/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/code/model/MotionDiffuse")
from MotionDiffuse.models.transformer_copy import MotionTransformer
...
class NextGPTModel(nn.Module):
    ...
    def __init__(self, **args):
        ...
        self.llama_model = LlamaForCausalLM.from_pretrained(self.vicuna_ckpt_path, device_map='auto')
        ...
        self.gen_text_hidden_fcs_motion = nn.ModuleList([])
        for layer_idx in self.args['text_emb_to_video_layers']:
            if layer_idx == -1 or layer_idx == self.llama_model.config.num_hidden_layers:
                in_dim = self.llama_model.config.hidden_size  # 4096
                self.gen_text_hidden_fcs_motion.append(
                    TextFcLayer(in_dim, 512, num_input_tokens=self.args['num_gen_video_tokens'],
                                num_output_tokens=self.args['num_clip_tokens'],
                                mode=self.args['text_fc_to_video_mode']))
            # self.vd_pipe.text_encoder.config.hidden_size
            elif layer_idx < self.llama_model.config.num_hidden_layers:
        ...
        self.gen_text_hidden_fcs_motion = self.gen_text_hidden_fcs_motion.to(self.device)
        ...
        for name, param in self.gen_text_hidden_fcs_audio.named_parameters():
            param.requires_grad = False
        for name, param in self.gen_text_hidden_fcs_motion.named_parameters():
            param.requires_grad = False
    ...
    def _add_motion_token(self):
        # self.llama_tokenizer.add_tokens({"<Aud>"})  # add special audio token to tokenizer
        # self.llama_tokenizer.add_tokens({"</Aud>"})  # add special audio token to tokenizer

        # Add [MOT] tokens to the vocabulary.
        self.args['gen_motion_token_idx'] = []
        for i in range(self.args['num_gen_motion_tokens']):
            print(f'Adding [MOT{i}] token to vocabulary.')
            print(f'Before adding new token, tokenizer("[MOT{i}]") =',
                  self.llama_tokenizer(f'[MOT{i}]', add_special_tokens=False))
            num_added_tokens = self.llama_tokenizer.add_tokens(f'[MOT{i}]')
            print(f'After adding {num_added_tokens} new tokens, tokenizer("[MOT{i}]") =',
                  self.llama_tokenizer(f'[MOT{i}]', add_special_tokens=False))
            gen_token_idx = self.llama_tokenizer(f'[MOT{i}]', add_special_tokens=False).input_ids
            assert len(gen_token_idx) == 1, gen_token_idx
            self.args['gen_motion_token_idx'].append(gen_token_idx[0])
    ...
    def Cosine_Similarity_Loss(self, output, target):
        ...
        return loss
    def _train_with_mode(self, texts, img_embeds=None, modality='motion', num_gen_tokens='24',
        ...
        if stage == 2:
            ...
            input_ids, target_ids, attention_mask = process_batch_stage_3(self.llama_tokenizer, texts, self.max_length,
                                                                          ...
                                                                          self.args['num_gen_motion_tokens']
                                                                          )
        else:
        ...
        if modality == 'text':
            ...
            if modality == 'image':
                ...
                mse_loss = l2_loss(embeddings, torch.stack(text_prompt_embeddins, dim=0).to(self.device)) 
                consine_loss = self.Cosine_Similarity_Loss(embeddings, torch.stack(text_prompt_embeddins, dim=0).to(self.device))
            else:
            ...
            if torch.isnan(mse_loss).any():
                mse_loss[torch.isnan(mse_loss)] = 0
            loss1 = loss + loss_scale * (mse_loss + consine_loss)
            with open('motion.txt', 'a') as f:
            # Ghi thêm một dòng vào file
                f.write(f"loss: {loss}, mse_loss: {mse_loss}, loss_sau:{loss1} \n")
            return loss1, gen_acc, mse_loss
    ...
    def _dec_align_training_stage_2(self, inputs):
        ...
        if dataset_type == 'TextToImage':
            ...
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            ...
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
        elif dataset_type == 'TextToMotion':
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            modality='motion',
                                                            num_gen_tokens=self.args['num_gen_video_tokens'],
                                                            text_hidden_fcs=self.gen_text_hidden_fcs_motion,
                                                            gen_token_idx=self.args['gen_video_token_idx'],
                                                            text_emb_layers=self.args['text_emb_to_video_layers'],
                                                            text_prompt_embeddins=inputs['caption_embs'],
                                                            stage=self.stage)
            # embeddings, texts = self._train_with_mode(texts=inputs,
            #                                                 modality='motion',
            #                                                 num_gen_tokens=self.args['num_gen_video_tokens'],
            #                                                 text_hidden_fcs=self.gen_text_hidden_fcs_motion,
            #                                                 gen_token_idx=self.args['gen_video_token_idx'],
    ...
    def generate_tokens_embeddings(self, inputs, input_embeds, temperature: float = 0.0, top_p: float = 1.0):
        ...
        outputs = self.llama_model.generate(
            inputs_embeds=input_embeds, #shape(1,20,4096)
        ...
        for _hidden_states in outputs.hidden_states[1:]:
            for idx in self.args['text_emb_to_img_layers']:
                ...
                # motion_output_embedding.append(_hidden_states[idx])
            ...
            for idx in self.args['text_emb_to_motion_layers']:
                motion_output_embedding.append(_hidden_states[idx])
        # motion_output_embedding.append(outputs.hidden_states[2][2])
        ...
        motion_output_embedding = torch.cat(motion_output_embedding, dim=1)
        

        return out, output_embeddings, video_output_embedding, audio_output_embedding, motion_output_embedding
    ...
    def generate_motions(self, generated_ids, embeddings, all_gen_idx):
        """
        To generate the images based on the embeddings
        generated_ids: the  index of the generated tokens
        embedding: the embeddings for synthesizing images
        all_gen_idx: the index of [MOT] in the generated_ids
        """
        last_ret_idx = 0
        return_outputs = []

        opt = get_opt(self.args['opt_path'],device= self.device)
        ...
        mean = np.load('/home/ltdoanh/jupyter/jupyter/ldtan/MotionDiffuse/t2m/t2m_new_ver4/meta/mean.npy')
        std = np.load('/home/ltdoanh/jupyter/jupyter/ldtan/MotionDiffuse/t2m/t2m_new_ver4/meta/std.npy')

        encoder = encoder.to(self.device)
        trainer = DDPMTrainer(opt, encoder)
        # trainer.load(pjoin(opt.model_dir, opt.which_epoch + '.tar'))
        trainer.load('/home/ltdoanh/jupyter/jupyter/ldtan/MotionDiffuse/t2m/t2m_new_ver2/model/latest.tar')
        ...
        for gen_idx in all_gen_idx:
            # assert generated_ids[0,
            #        gen_idx:gen_idx + self.args['num_gen_motion_tokens']].cpu().detach().numpy().tolist() == self.args[
            #            'gen_motion_token_idx'], (
            #     generated_ids[0, gen_idx:gen_idx + self.args['num_gen_motion_tokens']], self.args['gen_motion_token_idx'])
            raw_emb = embeddings[:, gen_idx - 1:gen_idx - 1 + self.args['num_gen_video_tokens'], :]  # (1, 8, 4096)

            # Produce generation embedding.
            # gen_prefix = ' '.join([f'[MOT{i}]' for i in range(self.args['num_gen_motion_tokens'])])
            gen_prefix = ''.join([f'[VID{i}]' for i in range(self.args['num_gen_video_tokens'])])

            gen_prefx_ids = self.llama_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
            gen_emb = self.gen_text_hidden_fcs_motion[-1](raw_emb, gen_prefix_embs)  # (1, 77, 768)
        ...
        print(gen_emb.shape)
        with torch.no_grad():
            if self.args['motion_length'] != -1:
                m_lens = torch.LongTensor([self.args['motion_length']]).to(self.device)
                print(m_lens.shape)
                pred_motions = trainer.generate(gen_emb, m_lens, opt.dim_pose, self.device)
                motion = pred_motions[0].cpu().numpy()
                motion = motion * std + mean
                title =  " #%d" % motion.shape[0]
                joint = recover_from_ric(torch.from_numpy(motion).float(), opt.joints_num).numpy()
                # joint = motion_temporal_filter(joint, sigma=1)
                plot_3d_motion(self.args['result_path'], paramUtil.t2m_kinematic_chain, joint, title=title, fps=20)
        return return_outputs
    ...
    def generate(self,inputs):
        ...
        generated_ids, generated_image_embeddings, generated_video_embeddings, generated_audio_embeddings, generated_motion_embeddings = self.generate_tokens_embeddings(
            inputs, input_embeds)
        ...
        # Find up to max_num_rest [MOT] tokens, and their corresponding scores.
        # all_gen_mot_idx = [i for i, x in enumerate(generated_ids[0, :] == self.args['gen_motion_token_idx'][0]) if x][
        #                   :inputs['max_num_vids']]
        # print('all_gen_mot_idx: ', all_gen_mot_idx)
        ...
        # all_gen_mot_idx.append(10)
        all_gen_mot_idx = [i for i, x in enumerate(generated_ids[0, :] == self.args['gen_video_token_idx'][0]) if x][
                          :inputs['max_num_vids']]
        print('all_gen_mot_idx: ', all_gen_mot_idx)

        if len(all_gen_img_idx) == 0 and len(all_gen_vid_idx) == 0 and len(all_gen_aud_idx) == 0 and len(all_gen_mot_idx)==0:
            ...
            # if len(all_gen_mot_idx) > 0:
            mot_outputs = self.generate_motions(generated_ids, generated_motion_embeddings, all_gen_mot_idx)
            return_outputs.append({'mot': mot_outputs})
        return return_outputs
