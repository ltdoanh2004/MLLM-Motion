...
class EvaluationDataset(Dataset):
    ...
    def __init__(self, opt, trainer, dataset, w_vectorizer, mm_num_samples, mm_num_repeats):
        ...
        args = {'model': 'nextgpt',
                'nextgpt_ckpt_path': '/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/delta_ckpt/nextgpt/7b_tiva_v0',
                'max_length': 128,
                'stage': 2,
                'root_dir': '/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT',
                'mode': 'validate',
                'pretrained_ckpt_path': '/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/pretrained_ckpt',
                }
        
        args.update(load_config(args))
        args['pretrained_ckpt_path']= '/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/pretrained_ckpt'
        self.model1 = NextGPTModel(**args)
        self.model1.stage = 2
        ...
        self.model1.load_state_dict(delta_ckpt, strict=False)

        delta_ckpt_new = torch.load('/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/test_11/pytorch_model.pt', map_location=torch.device('cuda'))
        ...
        # self.model1 = self.model1.eval().half().cuda()

        dataloader = DataLoader(dataset, batch_size=1, num_workers=1, shuffle=True)
        epoch, it = trainer.load('/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/checkpoint/fushion_checkpoint_8_gpu.tar')
        ...
        cur_idx = 0
        self.all_pred_motions = []
        my_list = [i for i in all_caption]  # Giả sử danh sách có 100 phần tử
        chunk_size = 30
        with torch.no_grad():
            # Duyệt qua từng phần
            for i in range(0, len(my_list), chunk_size):
                caption = my_list[i:i + chunk_size]
                m_lens = all_m_lens[i:i + chunk_size]
                # m_lens = torch.LongTensor([196]).to(opt.device)
                caption = [f'generate for me a video that {desc}' for desc in list(caption)]
                embeddings, texts = self.model1(caption)
                pred_motions = trainer.generate(embeddings, m_lens, opt.dim_pose, promt = texts)
                self.all_pred_motions.extend(pred_motions)
        cur_idx = 0
        mm_generated_motions = []
        with torch.no_grad():
            ...
            # for i in range(10):
                ...
                for t in range(repeat_times):
                    m_len = m_lens[0].item()
                    pred_motions = self.all_pred_motions[cur_idx][:m_lens[0].item()]
    ...
    def get_predict_motion(self):
        return self.all_pred_motions
...
class Text2MotionDatasetV2(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # Check if motion has NaN values initially
        assert not np.isnan(motion).any(), f"NaN detected in motion data for item {item}"
        ...
        pos_one_hots = np.concatenate(pos_one_hots, axis=0)
        word_embeddings = np.concatenate(word_embeddings, axis=0)
        assert not np.isnan(pos_one_hots).any(), f"NaN detected in motion data for item {item}"
        assert not np.isnan(word_embeddings).any(), f"NaN detected in motion data for item {item}"
        # Crop the motions into multiples of unit length, introduce small variations
        ...
        # Check if motion has NaN values after cropping
        assert not np.isnan(motion).any(), f"NaN detected in motion data after cropping for item {item}"

        # "Z Normalization"
        motion = (motion - self.mean) / self.std

        # Check if motion has NaN values after normalization
        assert not np.isnan(motion).any(), f"NaN detected in motion data after normalization for item {item}"
        ...
        # Final NaN check after padding
        assert not np.isnan(motion).any(), f"NaN detected in motion data after padding for item {item}"
...
def get_motion_loader(opt, batch_size, trainer, ground_truth_dataset, mm_num_samples, mm_num_repeats):
    ...
    gen_motion = dataset.get_predict_motion()
    ...
    return motion_loader, mm_motion_loader, gen_motion
...
class EvaluatorModelWrapper(object):
    ...
    def get_co_embeddings(self, word_embs, pos_ohot, cap_lens, motions, m_lens):
        with torch.no_grad():
            ...
            # Debugging for NaN values in input tensors
            assert not torch.isnan(word_embs).any(), "NaN detected in word_embs"
            assert not torch.isnan(pos_ohot).any(), "NaN detected in pos_ohot"
            assert not torch.isnan(motions).any(), "NaN detected in motions"

            # Sort based on m_lens
            ...
            assert not torch.isnan(movements).any(), "NaN detected in movements after movement_encoder"
            
            m_lens = m_lens // self.opt.unit_length
            motion_embedding = self.motion_encoder(movements, m_lens)
            assert not torch.isnan(motion_embedding).any(), "NaN detected in motion_embedding after motion_encoder"

            '''Text Encoding'''
            text_embedding = self.text_encoder(word_embs, pos_ohot, cap_lens)
            assert not torch.isnan(text_embedding).any(), "NaN detected in text_embedding after text_encoder"
