...
class MotionTransformer(nn.Module):
    ...
    def encode_text(self, text, device):
        ...
        print(text.shape)
        x = text + self.clip.positional_embedding.type(self.clip.dtype)
        ...
        xf_out = self.textTransEncoder(x)
        xf_out = self.text_ln(xf_out)
        xf_proj = self.text_proj(xf_out[text.argmax(dim=-2), torch.arange(xf_out.shape[1])])
        # B, T, D
        xf_out = xf_out.permute(1, 0, 2)
        return xf_proj, xf_out
...
if __name__== '__main__':
    doanh = MotionTransformer(input_feats=263)
    input = torch.rand(1, 77, 768)
    # input = 'doanh'
    x , y = doanh.encode_text(input, device=torch.device('cpu'))
    print(x.shape)
    print(y.shape)
