...
def print_current_loss(start_time, niter_state, losses, epoch=None, inner_iter=None):
    ...
    #         file.write('epoch: %3d niter: %6d inner_iter: %4d\n' % (epoch, niter_state, inner_iter))
    ...
    with open('log.txt', 'a') as file:
        if epoch is not None:
            epoch_info = 'epoch: %3d niter: %6d  inner_iter: %4d ' % (epoch, niter_state, inner_iter)
