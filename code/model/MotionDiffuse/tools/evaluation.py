...
from datasets1 import get_dataset_motion_loader, get_motion_loader, build_dataloader, EvaluationDataset
...
def score(trainers, gt_dataset, gen_motion, dim=3, mm_num_samples=100, mm_num_repeats=30):
    ...
    with torch.no_grad():
        max_length = max(tensor.size(0) for tensor in gen_motion)
        padded_predicted = [F.pad(tensor, (0, 0, 0, max_length - tensor.size(0))) for tensor in gen_motion]
        stacked = torch.stack(padded_predicted, dim=0).cpu().numpy()
        
        predicted = stacked * std + mean
    ...
    return average_mae, velocity_error, jerk_error, pae
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                ...
                assert not np.isnan(motions.cpu().numpy()).any(), "Motion embeddings contain NaN values"
    ...
    with open(log_file, 'w') as f:
        ...
        for replication in range(replication_times):
            ...
            for motion_loader_name, motion_loader_getter in eval_motion_loaders.items():
                motion_loader, mm_motion_loader, gen_motion= motion_loader_getter()
            ...
            print("BEGIN_____________________________________________________BEGIN", file=f, flush = True)
            print("_____________________________________________________________", file=f)
            print("MAE score", file=f)
            mae , velocity_error, jerk_error, pae = score(trainer, gt_dataset, gen_motion = gen_motion, dim =3)
            print(mae, file=f)
            print("_____________________________________________________________", file=f, flush = True)
            ...
            print(pae, file=f)
            print("END_____________________________________________________END", file=f, flush = True)
...
if __name__ == '__main__':
    ...
    dataset_opt_path = opt_path = '/home/ltdoanh/jupyter/jupyter/ldtan/MotionDiffuse/t2m/t2m_new_ver2/opt.txt'
    ...
    log_file = '/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/log_eval/t2m_evaluation.log'
    evaluation(log_file)
