...
class NextGPTModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.stage == 1:
            ...
            embeddings, texts = self._dec_align_training_stage_2(inputs)
        elif self.stage == 3:
    ...
    def generate_motions(self, generated_ids, embeddings, all_gen_idx, promt):
        """
        To generate the images based on the embeddings
        generated_ids: the  index of the generated tokens
        embedding: the embeddings for synthesizing images
        ...
        mean = np.load('/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/checkpoint/mean.npy')
        std = np.load('/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/checkpoint/std.npy')

        encoder = encoder.to(self.device)
        trainer = DDPMTrainer(opt, encoder)
        # trainer.load(pjoin(opt.model_dir, opt.which_epoch + '.tar'))
        trainer.load('/home/ltdoanh/jupyter/jupyter/ldtan/NExT-GPT/ckpt/checkpoint/fushion_checkpoint_8_gpu.tar')
        ...
        with torch.no_grad():
            if self.args['motion_length'] != -1:
                ...
                m_lens = torch.LongTensor([120]).to(self.device)
                pred_motions = trainer.generate(gen_emb, m_lens, opt.dim_pose, promt = promt)
    ...
    def generate(self, input,inputs):
        """
            inputs = {
        ...
        if len(all_gen_img_idx) == 0 and len(all_gen_vid_idx) == 0 and len(all_gen_aud_idx) == 0 and len(all_gen_mot_idx)==0:
            ...
            if len(all_gen_mot_idx) > 0:
                mot_outputs = self.generate_motions( generated_ids, generated_motion_embeddings, all_gen_mot_idx, promt = input)
