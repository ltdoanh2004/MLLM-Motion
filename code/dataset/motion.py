...
class MotionDataset(BaseDataset):
    ...
    def __init__(self, data_path: str, mm_root_path: str, embed_path: str, dataset_type: str):
        super(MotionDataset, self).__init__(data_path, mm_root_path, embed_path, dataset_type)
        self.embed_path = embed_path

        print('Load motion dataset ...')
        self.mm_path_list, self.caption_list = [], []
        with open(data_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        for row in tqdm(data, total=len(data)):
            image_id, one_caption = row["file_name"], row["caption"]
